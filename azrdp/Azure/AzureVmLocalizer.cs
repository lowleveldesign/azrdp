...
namespace LowLevelDesign.AzureRemoteDesktop.Azure
{
    sealed class AzureVMLocalizer
    {
        ...
        public async Task LocalizeVMAsync(string subscription, string resourceGroupName, string vmIPAddress, CancellationToken cancellationToken)
        {
            // subscription
            await FindSubscriptionAsync(subscription, cancellationToken);

            // resource group - we do not validate the name of the resource group
            if (!string.IsNullOrEmpty(resourceGroupName)) {
                this.resourceGroupName = resourceGroupName;
            } else {
                await PromptForResourceGroupAsync(cancellationToken);
            }
        ...
        private async Task PromptForResourceGroupAsync(CancellationToken cancellationToken)
        {
            Debug.Assert(subscriptionId != null);
